<template>
  <v-expansion-panels class="mt-6 mb-1" v-model="panel">
    <v-expansion-panel class="project-card">
      <template #title>
        ...
        <div v-else-if="instanceOfIProject($props.objects[0])">
          Search by <strong class="ml-1">Project</strong> and
          <strong class="ml-1">Organization</strong> name.
        </div>
        <div v-else-if="instanceOfIEnvironment($props.objects[0])">
          Search by <strong class="ml-1">Environment</strong>,
          <strong class="ml-1">Project</strong>, and
          <strong class="ml-1">Organization</strong> name.
        </div>
      </template>
      <template #text>
...
export default defineComponent({
  ...
  setup(props, { emit }) {
    ...
    const instanceOfIProject = (object: any): object is IProject =>
      'organization' in object
    const instanceOfIOrganization = (object: any): object is IOrganization =>
      'projects' in object
    const instanceOfIEnvironment = (object: any): object is IEnvironment =>
      'users' in object
    ...
    const performSearch = (query: string) => {
      ...
      if (instanceOfIProject(props.objects[0])) {
        filteredObjects.value = (props.objects as IProject[]).filter(
          (obj) =>
        ...
        filteredObjects.value = (props.objects as IOrganization[]).filter(
          (obj) => obj.title.toLowerCase().includes(searchLowerCase)
        )
      } else if (instanceOfIEnvironment(props.objects[0])) {
        filteredObjects.value = (props.objects as IEnvironment[]).filter(
          (obj) =>
            obj.name.toLowerCase().includes(searchLowerCase) ||
            obj.project.title.toLowerCase().includes(searchLowerCase) ||
            obj.project.organization.title
              .toLowerCase()
